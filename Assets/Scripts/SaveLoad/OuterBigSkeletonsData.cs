...
[System.Serializable]
public class OuterBigSkeletonData
{
    ...
    public int instantiatorMaxLevel;

    public OuterBigSkeletonData(Transform skeletonsHolder, CatapultArenaInstantiator catapultArenaInstantiator)
    {
        GetPositions(skeletonsHolder);
        GetRotations(skeletonsHolder);
        GetState(skeletonsHolder, catapultArenaInstantiator);
    }
    ...
    void GetPositions(Transform skeletonsHolder)
    {
        ...
        foreach (Transform skeleton in skeletonsHolder)
        {
            ...
            Debug.Log("saved one big outer skeleton positions");
        }
    }
    ...
    void GetRotations(Transform skeletonsHolder)
    {
        ...
        foreach (Transform skeleton in skeletonsHolder)
        {
            ...
            Debug.Log("saved one big outer skeleton rotations");
        }
    }

    void GetState(Transform skeletonsHolder, CatapultArenaInstantiator catapultArenaInstantiator)
    {
        instantiatorMaxLevel = catapultArenaInstantiator.CatapultsMaxCount;
        int indexer = 0;
