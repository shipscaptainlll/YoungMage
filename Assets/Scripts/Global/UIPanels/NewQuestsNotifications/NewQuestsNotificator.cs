...
public class NewQuestsNotificator : MonoBehaviour
{
    ...
    public void InstantiateQuestElement(string questDescription)
    {
        ...
        } else
        {
            ...
            newElement.localPosition -= new Vector3(0, elementSize.y, 0);
            questElements.Add(newElement);
            newElement.GetComponent<QuestNotificatorElement>().RecalibratePosition();
        }
        newElement.GetComponent<QuestNotificatorElement>().StartShowingElement();
        newElement.GetComponent<QuestNotificatorElement>().QuestDescription = questDescription;
    }
