...
public class HoverSettingsManager : MonoBehaviour
{
    [SerializeField] Transform graphicsElementsHolder;
    [SerializeField] Transform audioElementsHolder;
    [SerializeField] Transform controlsElementsHolder;
    [SerializeField] Transform miscElementsHolder;
    ...
    void Start()
    {
        foreach (Transform canvas in graphicsElementsHolder)
        {
            canvas.Find("Name").GetComponent<OnHoverHandler2D>().SettingsElementFound += Process;
            canvas.Find("Name").GetComponent<OnHoverHandler2D>().SettingsElementExited += Unprocess;
        }

        foreach (Transform canvas in audioElementsHolder)
        {
            canvas.Find("Name").GetComponent<OnHoverHandler2D>().SettingsElementFound += Process;
            canvas.Find("Name").GetComponent<OnHoverHandler2D>().SettingsElementExited += Unprocess;
        }

        foreach (Transform canvasRow in controlsElementsHolder)
        {
            foreach (Transform canvas in canvasRow)
            {
                canvas.Find("Name").GetComponent<OnHoverHandler2D>().SettingsElementFound += Process;
                canvas.Find("Name").GetComponent<OnHoverHandler2D>().SettingsElementExited += Unprocess;
            }
        ...
        foreach (Transform canvasRow in miscElementsHolder)
        {
            foreach (Transform canvas in canvasRow)
            {
                canvas.Find("Name").GetComponent<OnHoverHandler2D>().SettingsElementFound += Process;
    ...
    void Process(Transform element)
    {
        ...
        if (foundObject == null)
        {
            ...
            descriptionText.text = element.GetComponent<SettingsElement>().SettingsDescription;
